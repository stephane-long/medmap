...
def export_praticiens():
    ...
    # Restreindre à la zone (91=Essonne, 23=Creuse)
    df = df[df["Code postal (coord. structure)"].astype(str).str.startswith("23")]
    ...
    print(
        f"Export terminé : {out_file} ({nb_medecins} médecins · {nb_cabinets} cabinets)"
    )
