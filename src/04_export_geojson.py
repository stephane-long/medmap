...
def export_geojson():
    ...
    grid = gpd.read_file("data/grille_accessibilite_finale.gpkg")
    ...
    cols = [
        "h3_index", "temps_trajet_min",
        "praticien_adresse", "praticien_count", "praticiens_json",
        "geometry",
    ]
    grid = grid[[c for c in cols if c in grid.columns]]

    out_file = "data/grille_accessibilite.geojson"
